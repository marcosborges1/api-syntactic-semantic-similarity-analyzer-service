...
import asyncio
from knowledge_based import EdgeBased, FeatureBased, InformationContentBased
from corpus_based import WordEmbedding, NGD, LDA, LSA
from deep_learning import MyLSTM, MyBILSTM, MyTransformers
...
def default_metrics():
    # return ["lstm", "bilstm", "roberta", "mpnet"]
    return ["roberta", "mpnet"]
...
def load_dataset_by_api(path: str, api_o, api_t):
    ...
    return dataset[(dataset["origin_api"] == api_o) & (dataset["target_api"] == api_t)]
...
def compute_average_rank(dataset, metrics: list() = default_metrics()):
    ...
    dataset["avg_rank_sem"] = avg_ranks
...
def compute_major_rank(dataset, metrics: list() = default_metrics()):
    ...
    dataset["major_rank_sem"] = major_ranks

    return dataset


def compute_transformers_rank(dataset, metrics: list() = default_metrics()):
    # dataset = dataset.head(20)
    model_roberta = SentenceTransformer("roberta-base-nli-stsb-mean-tokens")
    model_mpnet = SentenceTransformer("stsb-mpnet-base-v2")

    dataset = dataset.copy()
    dataset["roberta"] = round(
        dataset.apply(
            lambda row: MyTransformers.calculate_roberta_similarity(
                row["oa_out_attr"], row["ta_in_attr"], model_roberta
            ),
            axis=1,
        ),
        3,
    ...
    dataset["mpnet"] = round(
        dataset.apply(
            lambda row: MyTransformers.calculate_mpnet_similarity(
                row["oa_out_attr"], row["ta_in_attr"], model_mpnet
            ),
            axis=1,
        ),
        3,
    ...
    # dataset["gpt"] = numpy.where(
    #     dataset[["roberta"]] + 0.05 >= 1,
    #     1,
    #     round(dataset[["roberta"]] + 0.05, 3),
    # )
...
def do_show_top_ordered_by_rank(
    dataset, by_param=["avg_rank_sem"], ascending_param=False, top=15
):
    # dataset = load_dataset()
    new_dataset = compute_average_rank(
        compute_gini_rank(compute_major_rank(compute_transformers_rank(dataset)))
    )
    # new_dataset = compute_transformers_rank(dataset)
    new_dataset = new_dataset[
        new_dataset["oa_out_attr_dt"] == new_dataset["ta_in_attr_dt"]
    ]
    new_dataset.to_csv("itemservice-fiscaldocumentservice.csv", index=False)
...
def do_show_top_ordered_best_metrics_by_rank(
    ...
    by_param=["avg_rank_sem"],
    ascending_param=False,
    top=15,
):
    # dataset = load_dataset()
    new_dataset = compute_average_rank(
        compute_gini_rank(compute_major_rank(dataset, metrics), metrics), metrics
    )
    new_dataset = new_dataset[
        new_dataset["oa_out_attr_dt"] == new_dataset["ta_in_attr_dt"]
    ]
...
async def resolve_semantic_similarities(_, info, path, ranking, threshold, k):
    """
    ...
    # Load dataset
    dataset = load_dataset(path)
    # columns = ["oa_out_attr", "ta_in_attr"]

    # print(dataset[columns].head(5).values.tolist(), columns)

    # data_frames = [
    #     dataset,
    #     await merge_deep_dt(dataset[columns].values.tolist(), columns),
    # ]
    # merged_df = reduce(
    #     lambda left, right: pd.merge(left, right, on=columns), data_frames
    # )
...
async def merge_deep_dt(word_list, columns):
    # lstm = MyLSTM("LSTM")
    # lstm.set_columns(columns)
    # lstm.analyze(word_list)

    # bi_lstm = MyBILSTM("BILSTM")
    # bi_lstm.set_columns(columns)
    # bi_lstm.analyze(word_list)

    transformer = MyTransformers("MyTransformers")
    transformer.set_columns(columns)
    transformer.analyze(word_list)

    data_frames = [
        # lstm.get_dataframe(),
        # bi_lstm.get_dataframe(),
...
async def merge_corpus_dt(word_list, columns):
    word_embedding = WordEmbedding("word Embedding")
    word_embedding.set_columns(columns)
    word_embedding.analyze(word_list)
    df = word_embedding.get_dataframe()
    return df


async def merge_kwnwonledge_dt(word_list, columns):
    edge = EdgeBased("word_embedding")
    feature = FeatureBased("word_embedding")
    information = InformationContentBased("word_embedding")

    edge.set_columns(columns)
    feature.set_columns(columns)
    information.set_columns(columns)

    edge.analyze(word_list)
    feature.analyze(word_list)
    information.analyze(word_list)

    data_frames = [
        edge.get_dataframe(),
        feature.get_dataframe(),
        information.get_dataframe(),
    ]
    merged_df = reduce(
        lambda left, right: pd.merge(left, right, on=columns), data_frames
    )
...
async def resolve_api_intergration_points(_, info, apiList, ranking, threshold, k):
    ...
    async with httpx.AsyncClient() as client:
        ...
        response1 = await client.post(
            f"{ase_service_endpoint}graphql",
            json={
                "query": """
        ...
        generatedExtractedFile = response1.json()["data"]["getExtractedApiList"][
            "generatedExtractedFile"
        ]

        # Call the second microservice
        response2 = await client.post(
            f"{adg_service_endpoint}",
            json={
                "query": """
            ...
            """,
                "variables": {"generatedExtractedFile": generatedExtractedFile},
            },
        )
        generatedDatasetFile = response2.json()["data"]["generateDataset"][
            "generatedDatasetFile"
        ]
        ...
        return {
            ...
            "semantic": resolve_semantic_similarities(
                _, info, generatedDatasetFile, ranking, threshold, k
            ),
        }
